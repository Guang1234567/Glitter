...
bool GLShader::setMatrix4(const std::string &name, GLsizei size, GLboolean transpose, const GLfloat* mat4ValuePtr) const
{
    GLint idx = glGetUniformLocation(mId, name.c_str());
    if (-1 != idx) {
        glUniformMatrix4fv(idx, 1, GL_FALSE, mat4ValuePtr);
      return true;
    }
    return false;
}
